...
export function CalculateSavings(
  ...
  for (let index = 1; index <= numOfYears; index++) {
    ...
    thisYearEnergyPurchaseCost =
      thisYearEnergyPurchaseCost * (1 + _params.annualEnergyPriceGrowth / 100);
    thisYearEnergySellingPrice =
      index === 1
        ? 0
        : thisYearEnergySellingPrice *
          (1 + _params.annualEnergyPriceGrowth / 100);
    const energyConstWithoutPV =
      thisYearEnergyPurchaseCost * _params.yearlyConsumptionWithoutPV;
    const selfConsumption =
      (_params.selfConsumptionRate / 100) * _params.annualEnergyProduced;
    const energyGivenBack = _params.annualEnergyProduced - selfConsumption;
    const energyConsumptionWithPV =
      _params.yearlyConsumptionWithoutPV - selfConsumption;
    const thisYearProfit = thisYearEnergySellingPrice * energyGivenBack;
    const energyCostWithPV =
      thisYearEnergyPurchaseCost * energyConsumptionWithPV - thisYearProfit;
...
export function CalculateEfficiency(numOfYears: number): SavingsRecord[] {
  const delta = 0.5 / 100;
  let efficiency = 100;
  ...
  for (let i = 2; i <= numOfYears; i++) {
    efficiency = efficiency - delta * efficiency;
    efficiencyChart.push({
      year: i,
      sum: efficiency,
    });
  }
